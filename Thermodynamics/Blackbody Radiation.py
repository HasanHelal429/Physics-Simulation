...
scm = []
for i in range(numP):
    scm.append(p1[i].x[0] * p1[i].y[0])

speed1 = [speed(p1, 0, 0)]
for i in range(numP):
    speed1.append(speed(p1, 0, i))
speedDist = [speed1]
...
for i in range(tIteration + 1):
    for j in range(numP):
        scm[j]= p1[j].x[i] * p1[j].y[i]
    scmInsertionSort(p1,scm)
    # This for loop runs over every point
    
    speedDist.append(speedDist[i])
...
#ani.save(filename="/Users/hasan/Python Animations/Billiard Balls.gif", writer="pillow",fps=50)
print(numCollisions)
plt.show()
